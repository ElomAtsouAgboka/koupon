...
export class CommandeService {
    ...
    protected convertDateFromClient(commande: ICommande): ICommande {
        const copy: ICommande = Object.assign({}, commande, {
            dateCommande:
                commande.dateCommande != null && commande.dateCommande.isValid() ? commande.dateCommande.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((commande: ICommande) => {
                commande.dateCommande = commande.dateCommande != null ? moment(commande.dateCommande) : null;
            });
        }
        return res;
    }
