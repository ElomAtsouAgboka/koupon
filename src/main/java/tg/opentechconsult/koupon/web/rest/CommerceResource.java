...
public class CommerceResource {
    ...
    public ResponseEntity<List<Commerce>> getAllCommerce(Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/commerce");
    ...
    /**
     * SEARCH  /_search/commerce?query=:query : search for the commerce corresponding
     * to the query.
     *
     * @param query the query of the commerce search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/commerce")
    public ResponseEntity<List<Commerce>> searchCommerce(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Commerce for query {}", query);
        Page<Commerce> page = commerceService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/commerce");
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
