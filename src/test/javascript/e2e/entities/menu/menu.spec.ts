...
describe('Menu e2e test', () => {
    ...
    it('should load Menus', async () => {
        ...
        expect(await menuComponentsPage.getTitle()).to.eq('kouponApp.menu.home.title');
    });

    it('should load create Menu page', async () => {
        await menuComponentsPage.clickOnCreateButton();
        menuUpdatePage = new MenuUpdatePage();
        expect(await menuUpdatePage.getPageTitle()).to.eq('kouponApp.menu.home.createOrEditLabel');
    ...
    it('should create and save Menus', async () => {
        ...
        await menuComponentsPage.clickOnCreateButton();
        await promise.all([
            menuUpdatePage.setMenuItemInput('menuItem'),
            menuUpdatePage.setMenuItemImgInput('menuItemImg'),
            menuUpdatePage.menuParentSelectLastOption()
        ]);
    ...
    it('should delete last Menu', async () => {
        ...
        expect(await menuDeleteDialog.getDialogTitle()).to.eq('kouponApp.menu.delete.question');
